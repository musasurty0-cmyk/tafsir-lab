...
# The screens the device shows, in the order they are reached.
SCREENS = [b["screen"] for b in BEATS if "screen" in b]
assert len(set(SCREENS)) == len(SCREENS), "a screen is used twice"
for s in SCREENS:
    assert (P / "assets" / (s + ".png")).exists(), "missing asset: " + s
...
screens_html = "\n".join(
    '            <img class="scr" id="scr-%s" src="assets/%s.png" alt="" '
    'data-layout-allow-overlap="true">' % (s, s) for s in SCREENS)
...
for b in BEATS:
    if "screen" in b:
        screen_js.append('        ["#scr-%s", %6.2f],' % (b["screen"], b["from"]))
...
%(screens_js)s
      ...
      /* Screens enter from the right, the way a push navigation does, in the
         order you would actually reach them: home, a reciter, the coverage
         wheel, the player. The one underneath slides out to the left, so the
         two move together rather than cross-fading through each other. */
      SCREENS.forEach(([sel, at], i) => {
        tl.set(sel, { opacity: 0, x: i === 0 ? 0 : 560 }, 0);
        tl.to(sel, { opacity: 1, x: 0, duration: i === 0 ? 0.01 : 0.52,
                     ease: "power3.out" }, at);
        if (i > 0) {
          const prev = SCREENS[i - 1][0];
          tl.to(prev, { x: -150, opacity: 0, duration: 0.52, ease: "power3.out" }, at);
        }
      });
...
dev_in = next(b["from"] for b in BEATS if "screen" in b) - 0.35
...
out = HTML % {
    ...
    "topBlocks": "\n".join(b for i, b in enumerate(blocks) if "screen" in BEATS[i]),
    "midBlocks": "\n".join(b for i, b in enumerate(blocks) if "screen" not in BEATS[i]),
    "enter": "\n".join(enter_js),
    "screens_js": "\n".join(screen_js),
    "devIn": dev_in,
    "devOut": dev_out,
...
print("wrote index.html  (%d beats, %d screens, %.2fs)" % (len(BEATS), len(SCREENS), DUR))
