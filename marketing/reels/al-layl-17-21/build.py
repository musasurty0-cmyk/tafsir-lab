"""Generate marketing/reels/al-layl-17-21/index.html from verses.json.
...
REC_END = doc["audio"]["duration"]      # 26.78
CLOSE_IN, DUR = 27.28, 29.28

MARK_COLOUR = "#9c3a2c"   # the rhyme word, once he reaches it

# Five ayat, the shortest two words and the longest six. Each is set on its own
# so the short ones are not timid and the long ones do not orphan a word.
SIZE = {17: 92, 18: 78, 19: 64, 20: 72, 21: 92}
...
# When he actually reaches the rhyme word, from word-level transcription, less
# the 1.12s trimmed off the head of the source.
#
# ALL FIVE are marked here, where Maryam left 87 plain. In al-Layl the rhyme is
# not decoration on the end of the line, it is the argument: al-atqa, who is
# spared it; yatazakka, why he gives; tujza, the repayment he is not chasing;
# al-a'la, whose face he is after instead; yarda, what he gets. Colour the five
# and you have read the passage.
MARK = {v["n"]: t for v, t in zip(V, [2.62, 6.72, 14.64, 22.08, 25.18])}
...
<html lang="en">
  <head>
    ...
    <style>
      /* ══════════════════════════════════════════════════════════════════
         al-layl-17-21 — 29.3s, 1080×1920, LIGHT.

         Sūrat al-Layl 17–21, recited, as ONE PAGE THAT SCROLLS.
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      /* The column is DELIBERATELY taller than this window and deliberately
         hangs outside it: that is what makes the passage one page being read
         rather than a stack of cards. The overflow is declared on the column
         itself — every āyah not currently at the reading line is outside the
         frame by design, and the mask is what makes that legible. */
      .viewport {
...
%(spine)s
        ...
        <div class="viewport" id="viewport">
          <div class="column" id="column" data-layout-allow-overflow>
%(blocks)s
          </div>
        </div>
        ...
        <div class="foot" id="foot">
          <span>Sūrat al-Layl · 92:17–21</span>
          <span>TafsirLab</span>
        </div>
      </div>
